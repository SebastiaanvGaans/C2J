...
public class Administratie {
    ...
    public Persoon addPersoon(Geslacht geslacht, String[] vnamen, String anaam,
        ...
        anaamClean = anaam.substring(0, 1).toUpperCase() + anaam.substring(1).toLowerCase();
        gebplaatsClean = gebplaats.substring(0, 1).toUpperCase() + gebplaats.substring(1).toLowerCase();
        String initialen = "";
        for (String firstChar : vnamenClean) {
            initialen += firstChar.charAt(0) + ".";
        }
        
        
        for(Persoon persoon : personen){
            if(persoon.getGebPlaats().equals(gebplaatsClean) && 
                    persoon.getGebDat().equals(gebdat) && 
                    persoon.getAchternaam().equals(anaamClean) &&
                    persoon.getTussenvoegsel().equals(tvoegsel.toLowerCase().trim()) &&
                    persoon.getInitialen().equals(initialen)
                    )
                return null;
        }
    ...
    public Gezin addHuwelijk(Persoon ouder1, Persoon ouder2, Calendar huwdatum) {
        ...
        for (Gezin gezin : gezinnen) {
            if (gezin.getOuder1() == ouder1 || gezin.getOuder2() == ouder1) {
                if (gezin.getOuder1() == ouder2 || gezin.getOuder2() == ouder2) {
                    if (gezin.isOngehuwd()) {
            ...
            if (persoon.getInitialen().toLowerCase().trim().equals(initialen.toLowerCase())) {
                if (persoon.getTussenvoegsel().toLowerCase().equals(tvoegsel.toLowerCase())) {
                    if (persoon.getAchternaam().toLowerCase().equals(anaam.toLowerCase())) {
                        if (persoon.getGebDat().equals(gebdat)) {
                            if (gebplaats == null) {
                                return persoon;
                            } else if (persoon.getGebPlaats().toLowerCase().equals(gebplaats.toLowerCase())) {
                                return persoon;
                            }
                        }
                    }
