...
public class Gezin {
    ...
    boolean setHuwelijk(Calendar datum) {
        //todo opgave 1
        if (this.isOngehuwd()) {
            if (this.ouder1.kanTrouwenOp(datum) && this.ouder2.kanTrouwenOp(datum)) {
    ...
    public String beschrijving() {
        //todo opgave 1
        String beschrijving = this.nr + " " + this.ouder1.getNaam() + " met " + this.ouder2.getNaam();
        if (!this.isOngehuwd()) {
            beschrijving += " " + StringUtilities.datumString(huwelijksdatum);
        }
        if (!this.kinderen.isEmpty()) {
            beschrijving += "; kinderen:";
            for (Persoon i : this.kinderen) {
                beschrijving += " -" + i.getVoornamen();
            }
        }
        return beschrijving;
    }
